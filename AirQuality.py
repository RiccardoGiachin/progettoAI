...
ph.columns = ['date', 'time', 'CO(GT)', 'PT08', 'NMHC', 'C6H6','PT08.S2', 'NOx', 'PT08.S3', 'NO2', 'PT08.S4', 'PT08.S5', 'T', 'RH', 'AH']
y = ph[['time']]
Y = [1 if fy.time == 18.00 else -1 for fy in y.itertuples()]
X = (ph.iloc[:, 0:14])
...
plt.scatter([x[0] for x in X_train], [X[1] for X in X_train], c=['g' if y is 1 else 'r' for y in Y_test])
plt.show()
